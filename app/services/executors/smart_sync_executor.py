...
class SmartSyncExecutor(JobExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            logger.info(f"原始配置: {work_config}")

            # 标准化字段名
            source_id = work_config.get('sourceId') or work_config.get('sourceDBId')
            target_id = work_config.get('targetId') or work_config.get('targetDBId')
            write_mode = work_config.get('syncMode', 'append')  # syncMode 实际是 writeMode

            logger.info(f"标准化后: sourceId={source_id}, targetId={target_id}, writeMode={write_mode}")
            ...
            # 从 targetColumns 构建字段信息
            target_columns = work_config.get('targetColumns', [])
            for col in target_columns:
                schema_mapping['columns'].append({
                    'name': col['code'],
                    'target_type': col.get('type', 'TEXT').upper(),
                    'nullable': True
                })
            ...
            logger.info(f"调用 smart_sync_service: {source_ds.name} -> {target_ds.name}")

            # 4. 调用 smart_sync_service 执行完整流程(自动建表+同步)
            ...
            #     namenode_port = os.getenv('HDFS_NAMENODE_PORT', '8020')
    ...
    async def validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 兼容 sourceId/sourceDBId
        if not config.get('sourceId') and not config.get('sourceDBId'):
            errors.append("缺少sourceId或sourceDBId")

        # 兼容 targetId/targetDBId
        if not config.get('targetId') and not config.get('targetDBId'):
            errors.append("缺少targetId或targetDBId")
